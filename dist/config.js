...
export async function loadConfig(cwd = process.cwd()) {
    ...
    if (!data)
        throw new Error("Missing config. Create reviewer.config.yaml");
    const parsed = schema.parse(data);
    const hydrated = await Promise.all((parsed.additional_context ?? []).map(async (item) => {
        if (item.path) {
    ...
    return { ...parsed, additional_context: hydrated };
}
