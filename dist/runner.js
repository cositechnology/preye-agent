...
import { buildContext, discoverImportNeighbors, loadNeighborSources, } from "./context.js";
import { reviewWithLLM } from "./llm.js";
import { anthropicProvider } from "./providers/anthropic.js";
import { openaiProvider } from "./providers/openai.js";
import { fetchPrFiles, postSummaryComment, fetchPrMeta, fetchFileAtRef, } from "./github.js";
...
import { parseUnifiedDiffPatch } from "./diff.js";
import { decide, formatDecision } from "./decision.js";
export async function runReview(opts) {
    const cfg = await loadConfig(opts.cwd);
    const provider = cfg.provider === "anthropic" ? anthropicProvider() : openaiProvider();
            ...
            if (cfg.context?.include_head_slices) {
                ...
                for (const f of selected) {
                    ...
                    for (const h of hunks) {
                        ...
                        const end = Math.min(lines.length, h.newStart + h.newLines - 1 + radius);
                        const snippet = lines
                            .slice(start - 1, end)
                            .map((t, idx) => `${(start + idx).toString().padStart(5, " ")}: ${t}`)
                            .join("\n");
                        let content = snippet;
                        if (content.length > maxChars)
                            content = content.slice(0, maxChars);
                        extraContextNotes.push(`### Context: ${f.filename}:${start}-${end}\n\n\`\`\`ts\n${content}\n\`\`\``);
                    }
                }
            }
        }
    ...
    try {
        if (cfg.graph?.enabled) {
            ...
            graphNotes = slices.map((s) => `# Symbol slice: ${s.path}:${s.startLine}-${s.endLine}\n\n\`\`\`ts\n${s.content}\n\`\`\``);
        }
    }
    catch {
        // fallback silently
    ...
    const llmFindings = await reviewWithLLM({
        provider,
        model: cfg.model,
...
function summarize(findings) {
    ...
    const lines = [
        ...
        ``,
        ...findings.map((f) => `- [${f.category}] (${f.severity}) ${f.file}:${f.startLine}-${f.endLine} — ${f.summary}`),
        ``,
