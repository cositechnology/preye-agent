...
export async function publishInlineComments(params: {
  ...
  const fileToAllowed = new Map<string, Set<number>>();
  ...
  for (const f of params.findings) {
    try {
      const allowed = fileToAllowed.get(f.file) || new Set<number>();
      ...
      console.error(
        `Failed to post comment for ${f.file}:${f.startLine}:`,
        error
      );
    }
  }
...
export async function publishReviewSummary(params: {
  ...
  const octokit = new Octokit({ auth: token || undefined });
  const event =
    params.decision.verdict === "Approved"
      ? "APPROVE"
