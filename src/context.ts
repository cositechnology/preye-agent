...
export function buildContext(
  files: PrFile[],
  additionalContext?: { prompt: string; context?: string }[]
): ReviewContext {
  const diffText = files
    ...
    .join("\n\n");
  const extraNotes = (additionalContext || []).map(
    (x) => `# ${x.prompt}\n${x.context || ""}`
  );
...
export function discoverImportNeighbors(
  filePaths: string[],
  cwd = process.cwd()
  ...
  for (const rel of filePaths) {
    ...
    while ((m = importRegex.exec(src))) {
      ...
      if (spec.startsWith(".")) {
        ...
        if (resolvedTs) deps.add(path.relative(cwd, resolvedTs));
      }
    }
...
function tryResolveTs(fromAbs: string, spec: string): string | null {
...
export function loadNeighborSources(
  neighborPaths: string[],
  opts?: { cwd?: string; maxFiles?: number; maxCharsPerFile?: number }
